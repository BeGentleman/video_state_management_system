...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label_5.setGeometry(QtCore.QRect(260, 0, 241, 241))
...
class query_window(QtWidgets.QMainWindow):
    ...
    def dianwole(self):
        ...
        # 然后将文件放到播放区域
        # TODO：改成路径自动生成
        file = "E:\\video_management_system\\" + i.text()
        print("file类型是{}".format(type(file)))
        url = (QtCore.QUrl.fromLocalFile(file))

        # 将封面缩略图显示在label5
        path_of_video_jpg = self.get_thumbnail_from_video(file)
        print("缩略图路径：", path_of_video_jpg)

        # 读取生成的图片路径
        # 先将path_of_video_jpg中的所有\变成/，否则读取不到图像，old_png为None就错了
        new_path_of_video_jpg = path_of_video_jpg.replace("\\", "/")
        old_png = cv.imread(new_path_of_video_jpg)

        # 对图像进行缩小处理
        new_png = cv.resize(old_png, None, fx=0.3, fy=0.3)
        # 将处理完的图片重新保存
        cv.imwrite(path_of_video_jpg.replace(".", "1."), new_png)
        final_path_of_video_jpg = path_of_video_jpg.replace(".", "1.")

        # 创建一个label承载缩略图
        # 调用QtGui.QPixmap方法，打开一个图片，存放在变量png中
        png = QtGui.QPixmap(final_path_of_video_jpg)

        self.ui.label_5.setPixmap(png)
    ...
    # 获取视频第一帧作为缩略图
    def get_thumbnail_from_video(self, video_path):
        thumbnail_path = video_path.replace(".mp4", ".jpg")
        ff = ffmpy.FFmpeg(
            executable='D:\\TongChouGuanLi_Develop_YU\\sources_management_system\\必须：视频解码器\\ffmpeg.exe',
